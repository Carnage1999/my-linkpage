...
- **自動偵測圖示** — 根據連結網址自動匹配圖示（支援 47+ 平台）；需要時可用 `iconSlug` 手動覆寫
- **動態 OG 圖片** — Vercel Serverless Function 依請求即時產生 1200×630 社群預覽圖，始終反映最新設定
...
- **分析與點擊熱力圖** — 可設定 Plausible / Umami 分析工具；本機端逐連結點擊熱力圖含每日/每週趨勢圖、強度指示器與重置功能，全部使用 `localStorage`，無 Cookie
- **SEO** — 動態 meta 標籤、Open Graph / Twitter Cards、JSON-LD Person schema、hreflang
- **錯誤邊界** — 頂層崩潰備援 UI，附重新載入按鈕
- **剪貼簿備援** — `navigator.clipboard` 搭配 `execCommand` 備援，相容舊版瀏覽器
- **無障礙** — skip link、語意化 HTML、ARIA 標籤、`aria-live` 回饋、WCAG 對比度
- **效能** — LazyMotion tree-shaking、Vite 手動 chunk 分割
- **測試** — Vitest + Testing Library 單元測試；Playwright E2E 測試（Chromium、Firefox、WebKit）
...
OG 圖片透過 `/api/og` **動態產生**，每次請求時讀取 `siteConfig.json`，無需手動重新產生。靜態備援圖片也會在 `pnpm build` 時透過 `scripts/generate-og-image.ts` 產生。

## 部署

已預先設定多個平台：

- **Vercel** — `vercel.json` 含安全標頭與資源快取
